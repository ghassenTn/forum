...
with cnt_news:
    ...
    if cols[0].button(f'{st.session_state["dislike_count"]} :material/thumb_down:', key='dislike',type='tertiary'):
    ...
    if cols[1].button(f'{st.session_state["like_count"]} :material/thumb_up:', key='like',type='tertiary'):
    ...
    with cols[2]:
        btn = st.button(f'{st.session_state["comment_count"]} :material/comment:', key='comment',type='tertiary')
    if btn:    
